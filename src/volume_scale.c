...
{
  // Nothing to do if we changed the scale value programmatically
...
static void create_volume_scale(void)
{
  ...
  g_signal_connect(G_OBJECT(scale), "value-changed",
                   G_CALLBACK(on_scale_value_change), NULL);
}
...
static void do_show_volume_scale(GdkRectangle *rect_or_null)
{
  ...
  if (rect_or_null)
  {
    ...
    gint monitor = gdk_screen_get_monitor_at_point(screen, rect_or_null->x,
                                                   rect_or_null->y);
...
static void on_pointer_press(GtkWidget *widget, GdkEventButton *event,
                             gpointer data)
{
...
void show_volume_scale(GdkRectangle *rect_or_null)
{
  ...
  g_signal_connect_after(G_OBJECT(window), "button_press_event",
                         G_CALLBACK(on_pointer_press), NULL);
