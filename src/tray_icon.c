...
static void on_scroll(GtkStatusIcon *status_icon, GdkEventScroll *event,
                      gpointer data)
{
  // Do nothing unless we have been updated at least once
  if (!updated_once)
...
void create_tray_icon(void)
{
  ...
  g_signal_connect(G_OBJECT(tray_icon), "activate", G_CALLBACK(on_activate),
                   NULL);
  g_signal_connect(G_OBJECT(tray_icon), "popup-menu", G_CALLBACK(on_menu),
                   NULL);
  g_signal_connect(G_OBJECT(tray_icon), "scroll_event", G_CALLBACK(on_scroll),
                   NULL);
...
void update_tray_icon(void)
{
  ...
  if (tooltip_text)
  {
    g_snprintf(tooltip_text, buffer_size, tooltip_text_format,
               (int)(as->volume));
