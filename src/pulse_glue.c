...
static void server_info_cb(pa_context *c, const pa_server_info *info,
                           void *data);
static void card_info_cb(pa_context *c, const pa_card_info *info, int eol,
                         void *data);
...
static gboolean postponed_sink_reload(gpointer data)
{
  ...
  sink_reload_operation = pa_context_get_sink_info_by_index(
      context, default_sink_index, sink_info_cb, NULL);
  if (!sink_reload_operation)
...
static void run_or_postpone_sink_reload(void)
{
  ...
  if (sink_reload_operation)
  {
    if (has_postponed_sink_reload)
      g_source_remove(postponed_sink_reload_timeout_id);
    postponed_sink_reload_timeout_id =
        g_timeout_add_seconds(1, postponed_sink_reload, NULL);
...
static void event_cb(pa_context *c, pa_subscription_event_type_t type,
                     uint32_t idx, void *data)
{
  ...
  case PA_SUBSCRIPTION_EVENT_CARD: {
    ...
    pa_operation *oper = pa_context_get_card_info_by_index(
        context, default_card_index, card_info_cb, NULL);
    if (oper)
      pa_operation_unref(oper);
    else
                         ...
                         void *data)
{
  ...
  gboolean default_card_changed =
      !have_default_card_index || default_card_index != info->card;
  ...
  if (!subscribed)
  {
    ...
    pa_operation *oper = pa_context_subscribe(context,
                           ...
                           void *data)
{
  ...
  sink_reload_operation = pa_context_get_sink_info_by_name(
      context, info->default_sink_name, sink_info_cb, NULL);
  if (!sink_reload_operation)
...
void pulse_glue_sync_volume(void)
{
  ...
  // Set the volume
  pa_operation *oper = pa_context_set_sink_volume_by_index(
      context, default_sink_index, &volume, NULL, NULL);
  if (oper)
    pa_operation_unref(oper);
  else
...
void pulse_glue_sync_muted(void)
{
  ...
  // Set the mute switch
  pa_operation *oper = pa_context_set_sink_mute_by_index(
      context, default_sink_index, shared_audio_status()->muted, NULL, NULL);
  if (oper)
    pa_operation_unref(oper);
  else
...
void pulse_glue_sync_active_profile(void)
{
  ...
  audio_status *as = shared_audio_status();
  uint32_t index = 0;
  for (GSList *entry = as->profiles; entry;
       entry = g_slist_next(entry), ++index)
  {
  ...
  pa_operation *oper = pa_context_set_card_profile_by_index(
      context, default_card_index, active_profile->name, NULL, NULL);
  if (oper)
    pa_operation_unref(oper);
  else
